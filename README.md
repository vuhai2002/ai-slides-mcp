...
| `cgimg accounts` shows "probe failed", or accounts suddenly 401 | OpenAI revoked the session, often after heavy/rapid checks (each `accounts`/probe = 3 backend calls). This is **not** a rate-limit - waiting won't help and force-refresh can't recover it. Re-run `cgimg login` for that account, and probe sparingly. |
