...
def _cmd_accounts(args: argparse.Namespace) -> int:
    ...
    if not store.load_accounts():
        print("Chưa đăng nhập account nào. Chạy `cgimg login`.")
        return 0
...
def _cmd_logout(args: argparse.Namespace) -> int:
    ...
    if args.all_accounts:
        store.remove_all()
        print("[OK] Đã đăng xuất tất cả account.")
        return 0
    if not args.selector:
        print("Cần <email|user_id> để xoá, hoặc --all để xoá hết.")
        return 2
    n = store.remove_account(args.selector)
    print(f"[OK] Đã xoá {n} account.")
    return 0
